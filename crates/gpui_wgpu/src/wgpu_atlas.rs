...
/// Upper bound on pending upload bytes held on the CPU before they are forced
/// out with an otherwise-empty submit. Mirrors the Metal backend's bound.
...
impl WgpuAtlas {
    ...
    pub fn clear(&self) {
        self.0.lock().clear(|textures| {
            ...
            textures.pending_upload_bytes = 0;
...
impl PlatformAtlas for WgpuAtlas {
    ...
    fn max_texture_size(&self) -> Option<Size<DevicePixels>> {
        let max = self.0.lock().backend.max_texture_size as i32;
        Some(Size {
